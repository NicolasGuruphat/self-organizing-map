...
class Neuron:
  ...
  def compute(self,x: numpy.array):
    '''
  ...
  def learn(self,eta,sigma,posxbmu,posybmu,x):
    ...
    @type x: numpy array
    '''
    delta = eta * exp(-(((pow(numpy.linalg.norm(numpy.array((posxbmu, posybmu)) - numpy.array((self.posx, self.posy))), 2))/(2 * pow(sigma, 2))))) * (x - self.weights)
    self.weights[:] += delta
  ...
  def espacement_neuronal(self):
    ...
    for i in range(len(self.map)-1):
      for j in range(len(self.map[i])):
        y += 1
        sum += numpy.linalg.norm(self.map[i][j].weights - self.map[i+1][j].weights)
    # addition horizontale
    for i in range(len(self.map)):
      for j in range(len(self.map[i])-1):
        y += 1
        sum += numpy.linalg.norm(self.map[i][j].weights - self.map[i][j+1].weights)
    return sum
# -----------------------------------------------------------------------------
